  ...
  try {
    ...
    const query = `
      SELECT
        ...
        po.total_value, po.total_value AS total_amount, po.compensation_amount, po.created_at, po.updated_at, po.staff_note, COUNT(pi.po_item_id) AS item_count,
      ...
      ${whereClause}
      GROUP BY po.po_id, po.po_code, po.supplier_id, s.name, po.created_by, creator.full_name, po.approved_by, approver.full_name, po.status, po.order_date, po.expected_delivery_date, po.received_date, po.total_value, po.compensation_amount, po.created_at, po.updated_at, po.staff_note
...
exports.getPurchasesDetail = async (req, res) => {
  try {
    ...
    const [discrepancies] = await pool.query(
      `SELECT * FROM po_discrepancies WHERE po_id = ?`, [id]
    );
...
exports.receiveOrder = async (req, res) => {
  ...
  try {
    ...
    await connection.query("UPDATE purchase_orders SET status = ?, received_date = CURRENT_TIMESTAMP, staff_note = ? WHERE po_id = ?", [newStatus, staffNote, id]);
    await connection.commit();
