...
const getOrCreateDefaultModel = async (connection) => {
    const [models] = await connection.query(
        `SELECT model_id FROM ml_models WHERE is_deployed = 1 ORDER BY training_date DESC, model_id DESC LIMIT 1`
    );
    if (models.length > 0) {
    ...
    const [result] = await connection.query(
        `
        INSERT INTO ml_models (version_tag, model_path, algorithm_type, hyperparameters, is_deployed)
        VALUES (?, ?, ?, ?, ?)
        `,
        [
            ...
            JSON.stringify({
                features: ['product_code', 'lag_1', 'lag_2', 'lag_3', 'target_month'],
...
// BE-01: Kết nối bảng suppliers để lấy số ngày lead_time_days quản lý thời gian giao hàng
const getForecastProducts = async (connection) => {
    const [products] = await connection.query(
        `
        SELECT
            p.product_id, p.sku, p.name, p.current_stock,
            p.min_stock_level, p.max_stock_level, p.cost_price, p.selling_price, p.is_discontinued,
        ...
        WHERE p.is_discontinued = 0
          AND (p.sku LIKE 'FOODS\\_%' OR p.sku LIKE 'HOBBIES\\_%' OR p.sku LIKE 'HOUSEHOLD\\_%')
        ORDER BY p.sku ASC
        `
    );
    return products;
...
const getLagFeaturesForProducts = async (connection, products, targetPeriod) => {
    ...
    const [rows] = await connection.query(
        `
        SELECT sd.product_id, DATE_FORMAT(st.transaction_date, '%Y-%m') AS month_key, SUM(sd.quantity) AS total_quantity
        FROM sale_details sd
        JOIN sales_transactions st ON sd.transaction_id = st.transaction_id
        WHERE sd.product_id IN (?) AND st.transaction_date >= ? AND st.transaction_date < ?
    ...
    products.forEach(product => {
        lagMap[product.product_id] = { lag_1: 0, lag_2: 0, lag_3: 0 };
    });
    ...
    rows.forEach(row => {
        ...
        if (monthKey === lag1Month) lagMap[productId].lag_1 = totalQuantity;
        if (monthKey === lag2Month) lagMap[productId].lag_2 = totalQuantity;
        if (monthKey === lag3Month) lagMap[productId].lag_3 = totalQuantity;
    });
...
const buildAiInputs = (products, lagMap, targetPeriod) => {
    ...
    return products.map(product => {
        const lags = lagMap[product.product_id] || { lag_1: 0, lag_2: 0, lag_3: 0 };
        return {
...
const buildFallbackPredictionMap = (products, lagMap) => {
    ...
    products.forEach(product => {
        const lags = lagMap[product.product_id] || { lag_1: 0, lag_2: 0, lag_3: 0 };
        const values = [Number(lags.lag_1 || 0), Number(lags.lag_2 || 0), Number(lags.lag_3 || 0)].filter(value => value > 0);
...
const predictProductsWithAI = async (products, lagMap, targetPeriod) => {
    try {
        ...
        predictions.forEach(prediction => {
            predictionMap[prediction.product_code] = Math.max(0, Number(prediction.predicted_quantity || 0));
        });
...
const getHistoricalData = async (connection, productId, targetPeriod) => {
    const [salesRows] = await connection.query(
        `
        SELECT DATE_FORMAT(st.transaction_date, '%Y-%m') AS month_key, DATE_FORMAT(st.transaction_date, '%b %Y') AS month_label, SUM(sd.quantity) AS total_qty,
            SUM(CASE WHEN sd.line_total IS NOT NULL THEN sd.line_total ELSE sd.quantity * sd.unit_price END) AS total_revenue
        FROM sale_details sd
        JOIN sales_transactions st ON sd.transaction_id = st.transaction_id
        WHERE sd.product_id = ? AND st.transaction_date < ?
        GROUP BY month_key, month_label ORDER BY month_key DESC LIMIT 6
        `,
...
// BE-01: Cải tiến thuật toán tính recommended_order dựa trên Lead Time Demand chuẩn quốc tế
const calculateProductForecast = async (connection, product, targetPeriod, predictedDemand) => {
    const historicalData = await getHistoricalData(connection, product.product_id, targetPeriod);
    ...
    const recommendedOrder = Math.max(0, Math.round(predictedDemand + leadTimeDemand + minStockLevel - currentStock));

    let stockStatus = 'normal';
    if (currentStock === 0) stockStatus = 'out';
    else if (currentStock < minStockLevel) stockStatus = 'low';
...
exports.getLatestForecast = async (req, res) => {
    ...
    try {
        ...
        if (products.length === 0) {
            return res.status(200).json({ success: true, target_period: targetPeriod, data: [] });
        }

        const lagMap = await getLagFeaturesForProducts(connection, products, targetPeriod);
        const predictionMap = await predictProductsWithAI(products, lagMap, targetPeriod);
        const forecastData = [];

        for (const product of products) {
            const predictedDemand = Number(predictionMap[product.sku] || 0);
            const forecast = await calculateProductForecast(connection, product, targetPeriod, predictedDemand);
            forecastData.push({ target_period: targetPeriod, ...forecast });
        }

        res.status(200).json({ success: true, target_period: targetPeriod, data: forecastData });
    } catch (error) {
...
// BE-02: Chỉnh sửa hàm để lưu chặt chẽ cột recommended_order vào Database bảng demand_forecasts
    ...
    try {
        ...
        for (const product of products) {
            ...
            const [existingRows] = await connection.query(
                'SELECT forecast_id FROM demand_forecasts WHERE product_id = ? AND target_period = ? LIMIT 1',
            ...
            if (existingRows.length > 0) {
                ...
                await connection.query(
                    ...
                    SET model_id = ?, forecast_date = CURRENT_TIMESTAMP, predicted_quantity = ?, lower_bound = ?, upper_bound = ?, recommended_order = ?
                    WHERE forecast_id = ?
                    `,
                    [modelId, forecast.predicted_quantity, forecast.lower_bound, forecast.upper_bound, forecast.recommended_order, forecastId]
                );
            } else {
                const [insertResult] = await connection.query(
                    `
                    INSERT INTO demand_forecasts (product_id, model_id, target_period, predicted_quantity, lower_bound, upper_bound, recommended_order)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    `,
                    [product.product_id, modelId, targetPeriod, forecast.predicted_quantity, forecast.lower_bound, forecast.upper_bound, forecast.recommended_order]
                );
                forecastId = insertResult.insertId;
            }

            savedForecasts.push({ forecast_id: forecastId, model_id: modelId, target_period: targetPeriod, ...forecast });
        }

        await connection.commit();
        await safeLogAction(getActorId(req), 'RUN_FORECAST', `Chạy dự báo AI nhu cầu nhập hàng kỳ ${targetPeriod}`, 'demand_forecasts', null, req.ip);

        res.status(200).json({ success: true, message: 'Chạy dự báo AI và lưu kết quả thành công', target_period: targetPeriod, data: savedForecasts });
    } catch (error) {
        await connection.rollback();
        console.error('Lỗi chạy dự báo:', error);
        res.status(500).json({ success: false, message: 'Lỗi server khi chạy dự báo' });
    } finally {
        connection.release();
    }
};

// BE-02: Đọc trực tiếp recommended_order đã lưu từ database, không tính động lại
exports.getSavedForecasts = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(
            `
            SELECT
                df.forecast_id AS id, df.forecast_id, df.product_id, p.sku, p.name AS product_name, COALESCE(c.name, 'General') AS category,
                df.model_id, m.version_tag, m.algorithm_type, df.forecast_date, df.target_period, df.predicted_quantity,
                df.predicted_quantity AS predicted_demand, df.lower_bound, df.upper_bound, p.current_stock, p.min_stock_level,
                df.recommended_order AS recommended_order
            FROM demand_forecasts df
            JOIN products p ON df.product_id = p.product_id
            LEFT JOIN categories c ON p.category_id = c.category_id
            LEFT JOIN ml_models m ON df.model_id = m.model_id
            ${whereSql}
        ...
        res.status(500).json({ success: false, message: 'Lỗi server khi lấy dữ liệu forecast đã lưu' });
    }
};
