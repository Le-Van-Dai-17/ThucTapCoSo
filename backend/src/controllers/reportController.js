...
exports.exportExcel = async (req, res) => {
    try {
        ...
        res.setHeader(
            'Content-Disposition',
            'attachment; filename="forecastai_report.xlsx"'
        );
...
exports.exportPDF = async (req, res) => {
    try {
        ...
        res.setHeader(
            'Content-Disposition',
            'attachment; filename="forecastai_report.pdf"'
        );

        doc.pipe(res);

        doc.fontSize(20).text('ForecastAI - Reports & Analytics', {
            align: 'center'
        });
        ...
        topProducts.forEach((p, index) => {
            doc.fontSize(10).text(
                `${index + 1}. ${p.name} | SKU: ${p.sku} | Sold: ${p.total_sold} | Revenue: ${Number(p.total_revenue).toLocaleString()} VND`
            );
        });
        ...
        categorySales.forEach((c, index) => {
            doc.fontSize(10).text(
                `${index + 1}. ${c.category} | Sold: ${c.total_sold} | Revenue: ${Number(c.total_revenue).toLocaleString()} VND`
            );
        });
...
exports.getSalesTrend = async (req, res) => {
    try {
        ...
        const data = salesTrend.map(item => ({
            date: new Date(item.sale_date).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric'
            }),
