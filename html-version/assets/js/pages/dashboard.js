...
function renderStockWarnings(rows) {
    ...
    body.innerHTML = rows.slice(0, 5).map(item => {
        ...
        return `<tr>
            ...
            ${isStaff ? '' : `<td class="px-5 py-3 text-center"><button onclick="createLowStockPO(${item.product_id})" class="px-3 py-1.5 rounded-lg border border-blue-200 text-blue-600 text-xs font-semibold hover:bg-blue-50 disabled:opacity-50" ${disabled}>T?o don nh?p</button></td>`}
        </tr>`;
    }).join('');
    updateCreateAllPOsButton();
}
