...
function renderTable() {
    ...
    filtered.forEach((order, i) => {
        ...
        // CHỐT NGHIỆP VỤ: Đơn đã Approved, Shipped, Received, Completed, Cancelled đều KHÔNG được phép sửa/xóa
        const canEdit = !isStaff && !['approved', 'shipped', 'received', 'completed', 'cancelled'].includes(order.status);
        const canApprove = !isStaff && order.status === 'pending'; // BE-04: Điều kiện hiện nút Duyệt cho Manager
...
document.getElementById('btnProceedReceive')?.addEventListener('click', async () => {
    ...
    try {
        // Đọc dữ liệu từ từng ô input thực đếm của Staff trên giao diện
