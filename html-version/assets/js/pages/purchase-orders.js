                    ...
                    <div class="flex flex-col gap-2 bg-gray-50 p-3 rounded-xl border border-gray-200 mb-2">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <input type="number" id="actualQtyInput_${idx}" min="0" max="${item.ordered_quantity}" value="${item.ordered_quantity}" 
                        ...
                        <div id="reasonContainer_${idx}" class="hidden w-full flex flex-col gap-1 mt-1 items-end">
                            <select id="reasonSelect_${idx}" class="text-xs border border-gray-300 rounded-lg px-2 py-1 focus:outline-none focus:border-blue-500 w-full max-w-[200px]" onchange="document.getElementById('reasonOtherContainer_${idx}').classList.toggle('hidden', this.value !== 'Other')">
                                <option value="" disabled selected>Select discrepancy reason...</option>
                            ...
                            <div id="reasonOtherContainer_${idx}" class="hidden w-full max-w-[200px]">
                                <input type="text" id="reasonOther_${idx}" placeholder="Enter specific reason..." class="w-full text-xs border border-gray-300 rounded-lg px-2 py-1 focus:outline-none focus:border-blue-500" />
                            </div>
                        </div>
                    </div>
                `).join('');
...
document.getElementById('btnProceedReceive')?.addEventListener('click', async () => {
    ...
    try {
        ...
        // Validation
        const exceedingItems = payloadItems.filter(p => {
        ...
        if (missingReasons.length > 0) {
            btn.disabled = false;
            btn.textContent = 'Confirm Receipt';
            showToast('Please select a discrepancy reason for items with differences!', 'error');
            return;
        }
        ...
        const imageFile = document.getElementById('receiveImage')?.files[0];
        if (imageFile) {
            formData.append('image', imageFile);
        }

        const res = await API.orders.receive(currentPOIdToReceive, formData);
